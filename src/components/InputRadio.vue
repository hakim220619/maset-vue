...
<template>
    <div :name="name" class="flex flex-wrap gap-4">
        <template v-if="lazyLoading">
            <Skeleton width="7rem" height="1.25rem" v-for="i in 3" :key="i" />
        </template>

        <div class="flex items-center gap-2" v-for="(item, index) in optionValues" :key="getKey(item, index)">
            <RadioButton v-model="data" :inputId="getId(item)" :value="getValue(item)" />
            <label :for="getId(item)" class="cursor-pointer">{{ getLabel(item) }}</label>
        </div>
    </div>
</template>
