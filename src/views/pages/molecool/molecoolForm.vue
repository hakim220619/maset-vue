...
const save = async () => {
    ...
    const url = route.params.id ? 'gateway/main_service/molecool/' + route.params.id : 'gateway/main_service/molecool';
    ...
    try {
        ...
        if (response.data.success) {
            swal.fire({
                title: 'Success',
                text: 'Ms Molecool saved successfully',
                icon: 'success'
            });

            router.push({ name: 'ms-molecool-list' });
        }
    } catch (error) {
...
onBeforeMount(async () => {
    ...
    if (route.params.id) {
        const userData = await Helper.getDataById('gateway/main_service/molecool', route.params.id);
        if (userData) {
...
const getSubTypes = () => fetchData('gateway/main_service/ms_molecool_types', getSubTypeData);

onMounted(async () => {
    const title = route.params.id ? 'Edit Ms Molecool' : 'Add Ms Molecool';
...
<template>
    <form class="card mt-8" @submit.prevent="save">
        <div class="grid md:grid-cols-2 md:gap-x-32 gap-y-4">
            ...
            <div class="grid md:grid-cols-4 gap-1 md:gap-4">
                <label for="ms_molecool_site_type_id" class="text-surface-500 dark:text-white">Subtype</label>
                <div class="md:col-span-3">
                    <Select v-model="form.data.ms_molecool_site_type_id" :options="getSubTypeData" show-clear
                        option-label="name" option-value="id" :virtualScrollerOptions="{ itemSize: 38 }" filter
                        placeholder="Select a Type" class="w-full" :invalid="!!form.errors.ms_molecool_site_type_id" />
                    <InputError :message="form.errors.ms_molecool_site_type_id" />
                </div>
            </div>
            <div class="grid md:grid-cols-4 gap-1 md:gap-4">
                <label for="name" class="text-surface-500 dark:text-white">Molecool Name</label>
                <div class="md:col-span-3">
