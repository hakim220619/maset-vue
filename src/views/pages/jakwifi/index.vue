...
async function getMsJakwifiSites() {
    isFiltering.value = true;

    const params = Helper.formatSearchParams(search.value);

    try {
        const response = await AuthApi.client().get('gateway/main_service/jakwifi/?' + new URLSearchParams(params));
...
const items = [
    {
        label: 'Ubah',
        command: (e) => {
            router.push({ name: 'ms-jakwifi-sites-edit', params: { id: e.item.data.id } });
        }
    },
    {
        label: 'Hapus',
    ...
    swal.mixin({
        ...
        .then(async (result) => {
            if (result.isConfirmed) {
                const response = await AuthApi.client().delete('gateway/main_service/jakwifi/' + id);

                if (response.data.success) {
                    swal.fire({
                        title: 'Success',
                        text: 'Ms Government deleted successfully',
                        icon: 'success'
                    });
...
const exportExcel = async () => {
    const params = Helper.formatSearchParams(search.value);
    await Helper.exportExcelFromApi('gateway/main_service/jakwifi/export', 'jakwifi.xlsx', params);
};

const add = () => {
    router.push('/pages/jakwifi/create');
    isRedirect.value = true;
};

onMounted(async () => {
    title.value = 'Ms Government';
});
</script>
    ...
    <div class="mt-4" v-if="showSearchFields">
        <div class="card space-y-4 mt-2 p-4 rounded-lg shadow-md">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div class="flex flex-col">
                    <label for="ms_jakwifi_site_type_id"
                        class="text-sm font-medium text-gray-700 dark:text-white">Search by Subtype</label>
                    <div class="relative">
                        <InputText v-model="search.ms_jakwifi_site_type_id" type="text"
                            class="mt-1 p-2 border rounded-md w-full" />
                        <button v-if="search.ms_jakwifi_site_type_id" @click="search.ms_jakwifi_site_type_id = ''"
                            class="absolute inset-y-0 right-2 text-gray-500 text-2xl">
                            &times;
                        </button>
                    </div>
            ...
            <div class="flex justify-end mt-6">
                <Button label="Search" @click="getMsJakwifiSites" :loading="isFiltering" raised
        ...
        <div class="card mt-5" v-if="visible">
            ...
            <div class="card mt-4">
                <DataTable :value="data" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]"
                    ...
                    <Column field="name" header="Government Name" style="width: 25%"></Column>
