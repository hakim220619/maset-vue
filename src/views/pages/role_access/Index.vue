...
const filters = ref({
    ...
    ra_name: { value: '', matchMode: 'contains' },
    ra_status: { value: null, matchMode: 'equals' },
    ra_created_at: { value: '', matchMode: 'contains' }
});
...
async function getRoleAccess() {
    ...
    try {
        const response = await AuthApi.client().get('/role_access/?' + new URLSearchParams(params));
        const filteredData = response.data.data;
        const sortedData = filteredData.sort((a, b) => {
            return new Date(b.ra_created_at) - new Date(a.ra_created_at);
        });
...
const items = [
    {
        label: 'Ubah',
        command: (e) => {
            router.push({ name: 'roleAccess-edit', params: { id: e.item.data.ra_id } });
        }
    },
    {
        label: 'Hapus',
        command: (e) => {
            destroy(e.item.data.ra_id, e.item.data.ra_name);
        }
    }
];
    ...
    swal
        ...
        .then(async (result) => {
            if (result.isConfirmed) {
                const response = await AuthApi.client().delete('role_access/' + id);
                if (response.data.success) {
                    swal.fire({
                        title: 'Success',
                        text: 'Role Access deleted successfully',
                        icon: 'success'
                    });
                    getRoleAccess();
                }
...
const exportExcel = async () => {
    const params = Helper.formatSearchParams(search.value);
    await Helper.exportExcelFromApi('gateway/idp/role_access/export', 'role_access.xlsx', params);
};

const add = () => {
    router.push('/pages/role_access/create');
    ...
    <div class="mt-4">
        <div class="card space-y-4 mt-2 p-4 rounded-lg shadow-md">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <StatusCard label="Total Access" :value="data?.length || 0" iconBg="bg-gray-100 dark:bg-gray-700"
                ...
                <StatusCard label="Active Access" :value="data?.filter(row => row.ra_status === 1).length || 0"
                ...
                <StatusCard label="Inactive Access" :value="data?.filter(row => row.ra_status === 2).length || 0"
