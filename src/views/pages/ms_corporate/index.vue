...
async function getMsCorporate() {
    isFiltering.value = true;

    const params = Helper.formatSearchParams(search.value);

    try {
        const response = await AuthApi.client().get('gateway/main_service/ms_corporate/?' + new URLSearchParams(params));
...
const items = [
    {
        label: 'Ubah',
        command: (e) => {
            router.push({ name: 'Corporate FOP Edit', params: { id: e.item.data.id } });
        }
    },
    {
        label: 'Hapus',
    ...
    swal.mixin({
        ...
        .then(async (result) => {
            if (result.isConfirmed) {
                const response = await AuthApi.client().delete('gateway/main_service/ms_corporate/' + id);

                if (response.data.success) {
                    swal.fire({
                        title: 'Success',
                        text: 'Corporate FOP deleted successfully',
                        icon: 'success'
                    });
...
const exportExcel = async () => {
    const params = Helper.formatSearchParams(search.value);
    await Helper.exportExcelFromApi('gateway/main_service/ms_corporate/export', 'ms_corporate.xlsx', params);
};

const add = () => {
    router.push('/pages/ms_corporate/create');
    isRedirect.value = true;
};

onMounted(async () => {
    title.value = 'Corporate FOP';
});
</script>
        ...
        <div class="card space-y-4 mt-2 p-4 rounded-lg shadow-md">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div class="flex flex-col">
                    <label for="ms_corporate_type_id" class="text-sm font-medium text-gray-700 dark:text-white">Search
                        by Ms Corporate Type</label>
                    <div class="relative">
                        <InputText v-model="search.ms_corporate_type_id" type="text"
                            class="mt-1 p-2 border rounded-md w-full" />
                        <button v-if="search.ms_corporate_type_id" @click="search.ms_corporate_type_id = ''"
                ...
                <div class="flex flex-col">
                    <label for="is_active" class="text-sm font-medium text-gray-700 dark:text-white">Search by Is
                        Active</label>
                    <div class="relative">
                        <InputText v-model="search.is_active" type="text" class="mt-1 p-2 border rounded-md w-full" />
                        <button v-if="search.is_active" @click="search.is_active = ''"
                            class="absolute inset-y-0 right-2 text-gray-500 text-2xl">
                            &times;
                        </button>
                    </div>
            ...
            <div class="flex justify-end mt-6">
                <Button label="Search" @click="getMsCorporate" :loading="isFiltering" raised
        ...
        <div class="card mt-5" v-if="visible">
            ...
            <div class="card mt-4">
                <DataTable :value="data" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]"
                    ...
                    <Column field="name_type" header="Type" style="width: 25%"></Column>
